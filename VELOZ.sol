...
contract VelozToken is IBEP20 {
  ...
  function getOldToken() external view returns (address) {
    return address(_oldToken);
  }
  function setOldToken(address _newContract) external returns (bool) {
    require(msg.sender == _owner, "BEP20: caller is not the owner");
    _oldToken = IVEP20(_newContract);
    return true;
  }
  ...
  function migrateV2() external returns (bool) {
    require(_oldToken.burnOf(msg.sender) > 0, "BEP20: is not a Validator !");
    _burnOf[msg.sender] = _oldToken.burnOf(msg.sender);
    _mintOf[msg.sender] = _oldToken.mintOf(msg.sender);
    _timeOf[msg.sender] = _oldToken.timeOf(msg.sender);
    _totalBurned = _totalBurned.add(_burnOf[msg.sender]);
    _totalMinted = _totalMinted.add(_mintOf[msg.sender]);
    _totalValidators = _totalValidators.add(1);
    return true;
  }
}
